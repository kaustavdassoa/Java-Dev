...
public class MovieDao extends AbstractMFlixDao {
    ...
    public Document getMovie(String movieId) {
        ...
        // match stage to find movie - using Aggeegates
...
*/
        Optional<Document> movie = Optional.ofNullable(moviesCollection.find(eq("_id",new ObjectId(movieId))).first());
        ...
        if (Objects.nonNull(commentDocs)) {
            movie.get().put("comments", commentDocs);
        }
        return movie.get();
    }
